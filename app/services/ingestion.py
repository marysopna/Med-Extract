...
import hashlib
from app.utils.doc_parser import parse_document, get_chunk_hash
...
def store_unique_chunks(chunks, vector_db_path="vector_store"):
    embeddings = OpenAIEmbeddings()
    unique_chunks = []
    seen_hashes = set()
...
async def process_upload(file: UploadFile):
    ...
    chunks = parse_document(tmp_path, ext)
    stored_chunks = store_unique_chunks(chunks)
    return [chunk.page_content for chunk in stored_chunks]